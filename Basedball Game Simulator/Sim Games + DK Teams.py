########################################
# Mark Max 10/05/20
# Baseball Game Simulator + Lineup Optimizer for Draftkings MLB Contests
# Designed in Pycharm Community Edition
#
# Drawing from an excel model I've created for expected plate appearance results by hitters & pitchers, this program
# will simulate each of the days baseball games a chosen # of times. After the simulation it will generate results csvs
# that can be used with a data analysis tool to potentially find sportsbetting advantages.
# After saving the game stats this program will then calculate ideal lineups for Draftkings DFS with a focus on high
# risk high potential lineups.
########################################
...
MIN_SCORE_TARGET = GlobalVariables.MIN_SCORE_TARGET
SCORE_TARGET_1 = GlobalVariables.SCORE_TARGET_1
SCORE_TARGET_2 = GlobalVariables.SCORE_TARGET_2
SCORE_TARGET_3 = GlobalVariables.SCORE_TARGET_3
SCORE_TARGET_4 = GlobalVariables.SCORE_TARGET_4
PLAYERS_PER_DK_TEAM = GlobalVariables.PLAYERS_PER_DK_TEAM
MAX_PLAYERS_PER_TEAM = GlobalVariables.MAX_PLAYERS_PER_TEAM
GAME_COUNT_TO_HALVE = GlobalVariables.GAME_COUNT_TO_HALVE
                                ...
                                for of1 in range(0, len(outfield) - 2):
                                    ...
                                    for of2 in range(of1 + 1, len(outfield) - 1):
                                        ...
                                        for of3 in range(of2 + 1, len(outfield)):
                                            ...
                                            teamids = [c_teamid, fb_teamid, sb_teamid, ss_teamid, tb_teamid,
                                                       of1_teamid, of2_teamid, of3_teamid]
...
def combineTeamsAndScores(hitterscores, pitcherscores, finaloutput):
    ...
    top_teams = len(dk_teams)

    for gameid in range(1, GAMES_TO_SIM + 1):

        for team in range(0, top_teams):
            p1_index = getPitcherStatsIndex(gameid, dk_teams[team][P1_TEAMID_INDEX], 1)
            p2_index = getPitcherStatsIndex(gameid, dk_teams[team][P2_TEAMID_INDEX], 1)
            c_index = getStatsIndex(gameid, dk_teams[team][C_TEAMID_INDEX], dk_teams[team][C_BATTING_ORDER_INDEX])
            fb_index = getStatsIndex(gameid, dk_teams[team][FB_TEAMID_INDEX], dk_teams[team][FB_BATTING_ORDER_INDEX])
            sb_index = getStatsIndex(gameid, dk_teams[team][SB_TEAMID_INDEX], dk_teams[team][SB_BATTING_ORDER_INDEX])
            ss_index = getStatsIndex(gameid, dk_teams[team][SS_TEAMID_INDEX], dk_teams[team][SS_BATTING_ORDER_INDEX])
            tb_index = getStatsIndex(gameid, dk_teams[team][TB_TEAMID_INDEX], dk_teams[team][TB_BATTING_ORDER_INDEX])
            of1_index = getStatsIndex(gameid, dk_teams[team][OF1_TEAMID_INDEX], dk_teams[team][OF1_BATTING_ORDER_INDEX])
            of2_index = getStatsIndex(gameid, dk_teams[team][OF2_TEAMID_INDEX], dk_teams[team][OF2_BATTING_ORDER_INDEX])
            of3_index = getStatsIndex(gameid, dk_teams[team][OF3_TEAMID_INDEX], dk_teams[team][OF3_BATTING_ORDER_INDEX])
...
#                                team[TOTAL_SALARY_INDEX]])
            ...
            dk_teams[team][TEAM_TOTAL_SCORE_INDEX] = dk_teams[team][TEAM_TOTAL_SCORE_INDEX] + total_score

            if total_score >= SCORE_TARGET_4:
                dk_teams[team][SCORE_TARGET_4_COUNT_INDEX] += 1
                dk_teams[team][SCORE_TARGET_3_COUNT_INDEX] += 1
                dk_teams[team][SCORE_TARGET_2_COUNT_INDEX] += 1
                dk_teams[team][SCORE_TARGET_1_COUNT_INDEX] += 1
                dk_teams[team][SCORE_TARGET_SCORE_INDEX] += 4
            if total_score >= SCORE_TARGET_3:
                dk_teams[team][SCORE_TARGET_3_COUNT_INDEX] += 1
                dk_teams[team][SCORE_TARGET_2_COUNT_INDEX] += 1
                dk_teams[team][SCORE_TARGET_1_COUNT_INDEX] += 1
                dk_teams[team][SCORE_TARGET_SCORE_INDEX] += 3
            if total_score >= SCORE_TARGET_2:
                dk_teams[team][SCORE_TARGET_2_COUNT_INDEX] += 1
                dk_teams[team][SCORE_TARGET_1_COUNT_INDEX] += 1
                dk_teams[team][SCORE_TARGET_SCORE_INDEX] += 2
            if total_score >= SCORE_TARGET_1:
                dk_teams[team][SCORE_TARGET_1_COUNT_INDEX] += 1
                dk_teams[team][SCORE_TARGET_SCORE_INDEX] += 1

        if gameid % 100 == 0:
            print("%f minutes - game " % ((time.time() - start_time) / 60) + str(gameid) + " dk scores recorded")

        if gameid % GAME_COUNT_TO_HALVE == 0:
            top_teams = math.ceil(top_teams/2)
            dk_teams.sort(key=sortScoreTarget, reverse=True)
            print("%f minutes - teams resorted" % ((time.time() - start_time) / 60))
...
def sortScoreTarget(val):
    return val[SCORE_TARGET_SCORE_INDEX]
